...
This is even more good news: basically you have available an arsenal of thousands of filters waiting to be used. The difference is the syntax: instead of the usual pipe filtering {% raw %}```{{ 'get salted' | sha512_digest }}```{% endraw %}, it would require writing {% raw %}```{{ salt.hashutil.sha512_digest('get salted') }}```{% endraw %} using the [hashutil function](https://docs.saltstack.com/en/2015.8/ref/modules/all/salt.modules.hashutil.html). Similarly {% raw %}```{{ salt.dnsutil.AAAA('www.google.com') }}```{% endraw %} from the [dnsutil module](https://docs.saltstack.com/en/2015.8/ref/modules/all/salt.modules.dnsutil.html) or {% raw %}```{{ salt.timezone.get_zone() }}```{% endraw %} to get the [timezone](https://docs.saltstack.com/en/2015.8/ref/modules/all/salt.modules.timezone.html) etc.
